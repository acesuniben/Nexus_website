  ...
  return (
    <div className="flex flex-col items-center md:flex-row justify-center gap-10 w-full">
      <div className="flex flex-col gap-2 items-start w-[90%] md:w-1/3 h-[100%]">
        {featured.imageUrl ? (
          <Image src={featured.imageUrl as string} alt={featured.title} width={427} height={226} className="rounded-xl"/>
        ) : (
          <div className="w-full h-[226px] bg-gray-100 rounded-xl flex items-center justify-center text-gray-400">No image</div>
        )}
      ...
      <div className="flex flex-col gap-5 md:gap-10 justify-between w-[90%] md:w-1/3 h-[100%]">
        {grid.map((item) => (
          <div key={item.id} className="h-[33%] grid grid-cols-[1fr 1fr 1fr 1fr] grid-rows-[1fr 1fr 1fr] gap-2 items-center justify-center">
            {item.imageUrl ? (
              <Image src={item.imageUrl as string} height={150} width={200} alt={item.title} className="rounded-xl col-start-1 col-end-3 row-start-1 row-end-4 w-[100%] h-[100%]" />
            ) : (
              <div className="rounded-xl col-start-1 col-end-3 row-start-1 row-end-4 w-[100%] h-[100%] bg-gray-100 flex items-center justify-center text-gray-400">No image</div>
            )}
